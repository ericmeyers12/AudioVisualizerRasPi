...
void swirlCycle(uint8_t wait) {
  uint16_t i=strip.numPixels()/2, j, k;
for(k=0; k<10; k++) {
  for(j=0; j<=(strip.numPixels()/2); j++) {
      strip.setPixelColor((i+j),BarColor(j));
      strip.setPixelColor((i-j),BarColor(j));
      strip.show();
      delay(wait);
  }
  for(j=(strip.numPixels()/2); j>0; j--) {
...
uint32_t BarColor(byte WheelPos) {
  ...
  if(WheelPos <= (NUM_PIXELS/3)) {
    ...
    return strip.Color(0, WheelPos * (255/(NUM_PIXELS/6)), 255 - WheelPos * (255/(NUM_PIXELS/6)));
  }
  WheelPos -= (NUM_PIXELS/3);
  return strip.Color(WheelPos * (255/(NUM_PIXELS/6)), 255 - WheelPos * (255/(NUM_PIXELS/6)), 0);
}
